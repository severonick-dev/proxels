...
- **Backend сам перезаливает.** Можно дёрнуть админ-эндпоинт «re-materialize
  all active subs», который пройдёт по всем активным подпискам и снова
  вызовет `addUser` для каждой. **TODO для следующей итерации.**
- **Перезапуск ноды → юзеры авто-перерегистрируются.** При следующем
  обращении клиента к `/api/sub/<token>` в `SubService` срабатывает lazy
  materialization, и UUID добавляется обратно. Но это работает только если
  клиент сам обновит подписку — а это происходит раз в 24ч по
  `Profile-Update-Interval`.

**Workaround на сейчас:** после рестарта Xray-ноды дёрнуть на RF:

```bash
sudo -u proxels psql -d proxels -c \
  'DELETE FROM "XrayClient" WHERE "nodeId" = (SELECT id FROM "Node" WHERE name = '"'"'de-1'"'"');'
```

Это удалит старые записи. При следующем обращении к sub-URL backend
re-materialize заново через gRPC AddUser. (Лучше сделаем это автоматическим
в админке.)
